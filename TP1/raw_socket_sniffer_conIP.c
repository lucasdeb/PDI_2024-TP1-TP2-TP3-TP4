...
void procesar_paquete(unsigned char *buffer, int size)
{
    ...
    if (encabezado_ip->protocol == IPPROTO_TCP)
    {
        ...
        unsigned int puerto_origen = ntohs(encabezado_tcp->th_sport);
        unsigned int puerto_destino = ntohs(encabezado_tcp->th_dport);
    ...
    else if (encabezado_ip->protocol == IPPROTO_UDP)
    {
        ...
        unsigned int puerto_origen = ntohs(encabezado_udp->uh_sport);
        unsigned int puerto_destino = ntohs(encabezado_udp->uh_dport);
...
int main()
{
    ...
    while (1)
    {
        printf("Esperando paquete... Socket: %d\n", sockfd);
        int bytes_recibidos = recvfrom(sockfd, buffer, sizeof(buffer), MSG_TRUNC, NULL, NULL);
